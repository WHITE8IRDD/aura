...
export function recordVisit(url: string, title: string, isPrivate = false): void {
  if (isPrivate) return
  if (!getSetting('rememberHistory')) return
...
export function updateTitle(url: string, title: string, isPrivate = false): void {
  ...
  if (!getSetting('rememberHistory')) return
  if (!title) return
  getDb().prepare('UPDATE history SET title = ? WHERE url = ?').run(title, url)
}

export function search(query: string, limit = 6): HistoryEntry[] {
  if (!getSetting('suggestHistory')) return []
  ...
  const candidates = db.prepare(
    `SELECT url, title, visited_at AS visitedAt, visit_count AS visitCount
     FROM history WHERE LOWER(url) LIKE ? OR LOWER(title) LIKE ? LIMIT 200`
...
export function deleteEntry(url: string): void {
  getDb().prepare('DELETE FROM history WHERE url = ?').run(url)
}
...
export function count(): number {
  const row = getDb().prepare('SELECT COUNT(*) AS c FROM history').get() as { c: number }
  return row.c
}
